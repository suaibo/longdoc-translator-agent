...
- **新建任务**：上传 PDF/Markdown/TXT，选择论文或小说模式。
- **任务概览**：状态、阶段、进度和 chunk 表。
- **术语审核**：术语表；写回能力在 TermService 完成后启用。
- **风险**：风险级别、类型、说明和原文片段。
- **输出**：双语 Markdown、中文 Markdown 和报告下载。

当前真实可用能力包括上传、OCR 模式、任务查询、轮询、取消、恢复、术语编辑确认、chunk/风险查看以及全部输出下载。
...
- 工作流事件时间线，展示时间、节点、状态、耗时和失败说明。
- 新建任务可选高风险 chunk/章节审核，人工审核页可批准当前待处理项。
...
- 定时轮询读取真实状态。
- 时间线能展示 `INTERRUPTED` 和 `FAILED` 节点。
- 审核批准后任务从原 LangGraph 中断点继续。
- 取消后状态变为 `CANCELLED`。
