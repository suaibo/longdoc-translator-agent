...
- 本地文件系统作为开发/Worker 工作目录；生产使用 S3 兼容对象存储持久化上传、解析和输出
...
- WebSocket 双向协作和多人协同审校。
- DOCX 导出。
- 完整重排版翻译 PDF（原始 PDF 保留与下载属于 MVP 输出增强）。
- 图片内容翻译。
- 翻译记忆库 TM。
- 向量数据库。
- RAG 问答。
- 支付或额度系统。
- 用户自定义模型供应商配置或在 UI 中填写 API Key。
- 小说模式完整实现。
...
- 不得把当前实现宣传为完整语义切分。
- 目标算法是“结构边界优先、DeepSeek 疑似语义边界判断、token 上限兜底”。
- DeepSeek 边界判断不可用时必须有限重试后回退规则基线，并记录风险。
...
4. 疑似边界发现与 DeepSeek CONTINUE/SPLIT/UNCERTAIN 判断。
...
## 19. 2026-06-25 当前权威增补

本节覆盖本文较早章节中与其冲突的旧 MVP 边界。

当前必须实现并已经进入代码基线：

- 简化用户名/密码账号、30 天可撤销会话、任务所有权和接口隔离。
- 用户关闭页面或退出登录后，独立 Worker 继续执行；重新登录可继续审核和下载。
- PostgreSQL 租约队列每用户最多并发 5 个任务，第 6 个排队。
- 本地开发存储与 S3 兼容对象存储；源文件、DocumentIR、资产和输出可跨进程恢复。
- 自动检测源语言，目标语言支持 `zh/en/ja/ko/fr/de/es/pt/ru/ar`。
- 不部署 embedding 服务。规则层先发现跨页断句、连字符、OCR 等疑点，只对疑点调用 DeepSeek。
- LangGraph 必须包含条件边、有限重试、规则降级、chunk 回环、人工 interrupt 和输出校验，不得表达为线性 pipeline。
- 高风险不等于任务失败：启用强制审核时中断；未启用时允许完成但必须显示 `hasUnresolvedRisks`。
- Gradio 不展示 term/review/chunk 等数据库 ID；风险显示问题、位置、原因、系统处理和建议。
- 任务概览显示队列位置、chunk 进度、ETA 和最后更新时间。
- 预翻译和风格 Prompt、服务器白名单模型选择、在线编辑与版本历史、SSE 事件流已经进入当前代码基线。

当前 Agent 主图与完整实施边界以 `docs/实施计划.md` 为准。

未来 TODO：翻译版/双语批注 PDF、图片文字翻译与导出、用户自定义模型供应商配置、多人协同编辑和 WebSocket 双向协作。
